...
![Mohr's circle representation of uniaxial loading (constant strain amplitude) at three points](../assets/chapter-7/mohr-circle-uniaxial-loading.png){#fig-mohr-circle-uniaxial-loading width=700 .lightbox}
...
![Nonproportional load path](../assets/chapter-7/mohr-circle-multiaxial-loading.png){width=700 .lightbox}

![Mohr's circle representation at eight points](../assets/chapter-7/multiaxial-loading-path.png){width=700 .lightbox}
...
![Applied vs. total shear strain in proportional and nonproportional loading paths](../assets/chapter-7/applied-shear-strain-total-shear-strain-comparison.png){#fig-applied-total-shear-strain-comparison width=600 .lightbox}
...
These theories rely on a linear stress-strain relation, meaning they are limited to the **high cycle fatigue** (HCF) regime where the stresses are mostly elastic.

For each theory, $\sigma_i$ and $\varepsilon_i$ denotes principal stress and strain, are are ordered so that $\sigma_1\geq\sigma_2\geq\sigma_3$ and $\varepsilon_1\geq\varepsilon_2\geq\varepsilon_3$. Additionally, define $\sigma_e$ and $\varepsilon_e$ as the effective stress and strain.
...
$$
\begin{aligned}
\sigma_e & =\sqrt{\frac {(\sigma_x-\sigma_y)^2+(\sigma_y-\sigma_z)^2+(\sigma_z-\sigma_x)^2+6\left(\tau_{xy}^2+\tau_{yz}^2+\tau_{xz}^2\right)}2} \\
