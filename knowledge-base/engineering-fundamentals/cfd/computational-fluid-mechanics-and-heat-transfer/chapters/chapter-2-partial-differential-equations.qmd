...
Consider the general PDE below, where the coefficients only depend on $x$ and $y$ so @eq-pde-general-form is linear.
...
@eq-pde-general-form-rearranged becomes
...
@eq-pde-general-form-rearranged-tau and @eq-phi-tau-chain-rule form a $3\times3$ system to calculate second order derivatives from $\phi$ and its first derivatives along $C$.
...
The curves $y(x)$ that satisfy @eq-pde-general-characteristics-differential are the *characteristics* of the PDE. The discriminant $\Delta\equiv b^2-4ac$ plays an important role in determining the PDE's classification.
