...
Rewriting Eqn. (@eq-mean-value-theorem-slope) as a line in point-slope form, then
...
Using Eqn. (@eq-mean-value-theorem-point-slope) allows us to rewrite $\Delta y_4$ in terms of $\Delta x$. Since the theorem only *guarantees* that a point will exist (i.e., the actual value of the point is still unknown), let $c_4$ be this special point. From the Mean Value Theorem, then
...
**Answer:** Since $y$ is given in terms of $x$, we should integrate across $x$ (i.e., use the first equation in Eqn. (@eq-arc-length-integral)). Using the fact that the derivative of $x^n$ is $nx^{n-1}$, and that the derivative of $\ln x$ is $\frac 1x$, the derivative of $y(x)$ is
...
**Answer:** Since $x$ is given in terms of $y$, we should use the second equation in Eqn. (@eq-arc-length-integral). Since $y=\frac 13y^{3/2}-y^{1/2}$, then the derivative is $\frac {\mathrm dx}{\mathrm dy}=\frac 12y^{1/2}-\frac 12y^{-1/2}=\frac 12\left(y^{1/2}-y^{-1/2}\right)$. The integrand is
